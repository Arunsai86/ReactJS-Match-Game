...
class App extends Component {
  ...
  getFilterImages = tabId => {
    const {displayImages} = this.state
    const filterImages = displayImages.filter(
      eachImg => eachImg.category === tabId,
    )
    return filterImages
  }

  onClickThumbnailImg = id => {
    const {randomImgItem} = this.state
    if (randomImgItem.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        randomImgItem:
          imagesList[Math.round(Math.random() * imagesList.length)],
      }))
    } else {
      clearInterval(this.intervalID)
      this.setState({isTrue: true})
  ...
  render() {
    const {isTrue, activeTabId, randomImgItem, score, timer} = this.state
    // console.log(activeTabId)

    const filterImagesList = this.getFilterImages(activeTabId)
        ...
        {isTrue ? (
          <div className="result-container">
            <div className="container">
              ...
              <h1 className="result-heading">{score}</h1>
              <div className="play-again-container">
                ...
                <button
                  ...
                  onClick={this.clickPlayAgainBtn}
                  type="button"
                >
                  Play Again
                </button>
          ...
          <div className="game-page">
            ...
            <ul className="images-container">
              {filterImagesList.map(eachImgItem => (
                <ImageItem
                  eachImgItem={eachImgItem}
